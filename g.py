...
async def run_attack_command_on_codespace(target_ip, target_port, duration):
    command = f"./sam {target_ip} {target_port} {duration} 100"
    try:
...
def handle_message(message):
    ...
    if message.text == "Instant Plan 🧡":
        ...
        bot.reply_to(message, "*CLICK INSTANT ++ AND SEND IP PORT AND TIME 120*", parse_mode='Markdown')
    elif message.text == "Contact admin✔️":
        bot.reply_to(message, "*@ILLEGALCHEAT78*", parse_mode='Markdown')
    else:
